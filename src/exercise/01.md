...
1. I learnt how the useReducer hook works. it works almost like the useState
   hook works. the useReducer hook accept three argument, the first argument is
   the reducer function, the second is the initial state and the the third is
   the lazy initializer function. the useReducer hook also returns two values
   just like useStat hook the first in the state and the second is the mechanism
   to call the reducer function which is commonly know as dispatch. the reducer
   function updates the state base on some actions. the first argument of the
   reducer function is the current state the section is the action.
