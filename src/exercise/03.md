...
1. The context API is used to share state across deferent components in your
   react app.

2. You first create the context you want to use using React.createContext. The
   createContext receive and argument for the default value.

3. Context Provider is used to pass the context value across components you wrap
   component that needs to use the context value with the context provider. all
   the component inside a context provider has access to the context values.
   When the nearest context provider changes it triggers the re-render.

4. useContext hooks is used to get the context values. All the component calling
